    ...
    public static int RegisterMyService(Context context, String name, int[] roapPort, int[] airPlayPort) {
        ...
        try {
            ...
            if (mWifiManager.getWifiState() == WifiManager.WIFI_STATE_ENABLED) {
                ...
                if (connInfo == null) {
                    log.d("Failed to get connection info");
                    return -2;
                }
                int myRawAddress = connInfo.getIpAddress();
                myAddressBytes = new byte[]{(byte) (myRawAddress & 0xff), (byte) (myRawAddress >> 8 & 0xff), (byte) (myRawAddress >> 16 & 0xff), (byte) (myRawAddress >> 24 & 0xff)};
                macAddress = connInfo.getMacAddress();
                try {
                    myAddress = InetAddress.getByAddress(myAddressBytes);
                } catch (UnknownHostException e) {
                    log.d("Failed to get address: " + e.toString());
                    return -3;
                }
            }
            ...
            if (myAddress != null) 
            {
                NetworkInterface ni = NetworkInterface.getByInetAddress(myAddress);
                if (ni == null) 
                {
                	macAddress = null;
                	log.d("========Failed to get address============ " );
                }
            }
          
            
            if (mWifiManager.getWifiState() != WifiManager.WIFI_STATE_ENABLED || macAddress == null || "".equals(macAddress.trim())) {
                log.d("Wifi state is not enabled");
                //return -1;

                List<NetworkInterface> interfaces = Collections.list(NetworkInterface.getNetworkInterfaces());
                for (NetworkInterface intf : interfaces) {
                    //只索引WiFi网络的IP，一般手机只会有一个wlan，即wlan0（少数手机WiFi Ap模式下的接口为“ap0”）
                    if(!intf.getDisplayName().startsWith("wlan") && !intf.getDisplayName().startsWith("ap0") && !intf.getDisplayName().startsWith("eth"))
                        continue;
                    ...
                    for (InetAddress addr : addrs) {
                        if (!addr.isLoopbackAddress()) {
                            ...
                            if (isIPv4) {
                                ...
                                //myAddressBytes = addr.getAddress();
                                break;
                            }
                        }
                    }
            ...
            macAddress = getRandom();//"50:38:3c:a8:98:34";
